...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    try {
        ...
        $query_parameters = array(
            ...
            'order' => request::body('order', false, true, FILTER_DEFAULT, array('options' => array('default' => 'periodID'))),
        ...
        response::result_info(db::init(USER_ID)->num_rows(), db::init(USER_ID)->num_rows_all('period'), $query_parameters['page'], $query_parameters['per_page']);
...
// Modify existing periods
    ...
    try {
        ...
        db::init(USER_ID)->run_query($query, $sql_parameters[0], ...array_slice($sql_parameters, 1));
        db::init(USER_ID)->run_query("SELECT * FROM `period` WHERE `periodID`=?", "i", $path_parameters['id']);

        response::result(db::init(USER_ID)->fetch_one());
        response::result_info(db::init(USER_ID)->num_rows(), db::init(USER_ID)->num_rows_all('period'));
    } catch (JsonException $e) {
        response::error('Faulty request data. JSON ' . $e->getMessage(), $e->getCode());
        response::send(false, 400);
        exit;
...
// Create new period
    ...
    try {
        ...
        db::init(USER_ID)->run_query("INSERT INTO `period` (`label`, `active`) VALUES (?, ?);", "ss", $query_parameters['label'], $query_parameters['active'] ? 'Y' : 'N');
        db::init(USER_ID)->run_query("SELECT * FROM `period` WHERE `periodID`=?", "i", db::init(USER_ID)->insert_id);
...
// Delete period
    ...
    try {
        ...
        db::init(USER_ID)->run_query("DELETE FROM `period` WHERE `periodID`=?", "i", $path_parameters['id']);

        if (db::init(USER_ID)->affected_rows == 0) {
            response::result(array('periodID' => null));
            throw new ApplicationRuntimeException("Record with id " . $path_parameters['id'] . " not found. No period deleted.");
        } else {
            response::result(array('periodID' => $path_parameters['id']));
        }
    } catch (RequestException $e) {
        response::error($e->getMessage(), $e->getCode());
        response::send(false, 400);
