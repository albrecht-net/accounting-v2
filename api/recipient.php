...
if ($_SERVER['REQUEST_METHOD'] == 'GET') {
    ...
    try {
        ...
        $query_parameters = array(
            ...
            'order' => request::body('order', false, true, FILTER_DEFAULT, array('options' => array('default' => 'recipientID'))),
        ...
        response::result_info(db::init(USER_ID)->num_rows(), db::init(USER_ID)->num_rows_all('recipient'), $query_parameters['page'], $query_parameters['per_page']);
...
// Modify existing recipients
    ...
    try {
        ...
        $query .= " WHERE `recipientID`=?";
        $sql_parameters[0] .= "i";
        $sql_parameters[] = $path_parameters['id'];

        db::init(USER_ID)->run_query($query, $sql_parameters[0], ...array_slice($sql_parameters, 1));
        db::init(USER_ID)->run_query("SELECT * FROM `recipient` WHERE `recipientID`=?", "i", $path_parameters['id']);

        response::result(db::init(USER_ID)->fetch_one());
        response::result_info(db::init(USER_ID)->num_rows(), db::init(USER_ID)->num_rows_all('recipient'));
    } catch (JsonException $e) {
        response::error('Faulty request data. JSON ' . $e->getMessage(), $e->getCode());
        response::send(false, 400);
        exit;
...
// Create new recipient
    ...
    try {
        ...
        db::init(USER_ID)->run_query("INSERT INTO `recipient` (`label`, `active`) VALUES (?, ?);", "ss", $query_parameters['label'], $query_parameters['active'] ? 'Y' : 'N');
        db::init(USER_ID)->run_query("SELECT * FROM `recipient` WHERE `recipientID`=?", "i", db::init(USER_ID)->insert_id);
...
// Delete recipient
    ...
    try {
        ...
        db::init(USER_ID)->run_query("DELETE FROM `recipient` WHERE `recipientID`=?", "i", $path_parameters['id']);

        if (db::init(USER_ID)->affected_rows == 0) {
            response::result(array('recipientID' => null));
            throw new ApplicationRuntimeException("Record with id " . $path_parameters['id'] . " not found. No recipient deleted.");
        } else {
            response::result(array('recipientID' => $path_parameters['id']));
        }
    } catch (RequestException $e) {
        response::error($e->getMessage(), $e->getCode());
        response::send(false, 400);
